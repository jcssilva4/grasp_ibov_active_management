...
for t = 2:period
    ...
    fprintf('ok!\ncomputing turnover...');
    %use this if you don't have an initial solution
    if(t>2)
        PerformanceMetrics{2,3}(t-1) = get_turnover_t(optPortfolios{1,t-1}, optPortfolios{1,t});
    else
        PerformanceMetrics{2,3}(t-1) = 1; %you will buy assets so that wi sum up to 1
    end
    % PerformanceMetrics{2,3}(t-1) = get_turnover_t(optPortfolios{1,t-1},% optPortfolios{1,t});%use this if you have an initial solution
    PerformanceMetrics{3,3}(t-1) = get_turnover_t(benchPortfolios{1,t-1}, benchPortfolios{1,t}); %bench
    fprintf('ok!\ncomputing adjusted return...');
    PerformanceMetrics{2,4}(t-1) = PerformanceMetrics{2,2}(t-1) - 0.00032*PerformanceMetrics{2,3}(t-1);
    PerformanceMetrics{3,4}(t-1) = PerformanceMetrics{3,2}(t-1) - 0.00032*PerformanceMetrics{3,3}(t-1); %bench
