function [S, Sc, proceedGRASP] = adjustSecMcapQ(P, S, Sc, RCL)
    ...
    if(sec_ok + mcap_ok > 1) %check if both are satisfied
        ...
        if(~mcap_ok) %search the missing mcapq in RCL
            for s=1:length(RCL)
                currentmcapq = P.initialAssetList{1,RCL(s)}.mCapQ;
                if(~sum(ismember(currentmcapq,mcapqSet)))
                    mcapqSet = [mcapqSet currentmcapq]; %add new mcapq
                    S = [S RCL(s)]; %add a new stock containing one of the remaining mcapq
                    Sc = setdiff(Sc, RCL(s)); %Sc - s
                end
            end
        end
