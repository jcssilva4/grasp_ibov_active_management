function [l, Q, r] = getNonlinCon_qcqp(Portfolios, COVmat, S, t0, tf)
    %constraint: (Risk_port < min Riskbench from the last 3 rebalance periods)
    Q = zeros(2*length(S));
    for i = 1:length(S)
        for j = 1:length(S)
            Q(i+length(S),j+length(S)) = COVmat(i,j);
        end
    end
    l = []; 
    r = get_historical_MinRisk(Portfolios, t0, tf); 
    %{
