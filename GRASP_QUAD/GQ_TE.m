%% solves 
function [w, fval, Sfinal, COVIJ, covij] = GQ_TE(P, iterMAX, asset_num, teta)
%{
    Some problems:
...
%}
  %read files
    ...
    while (itrGRASP <= itrMAX_GRASP)
        K = asset_num; %size of the solution (number of stocks to be selected)% remember that: 50<=K<=70 (r9.1)
        t0 = cputime; %tic
        sizeRCL = SIZERCL;
        ...
        while (length(S) < K) %K is the number of stocks to be selected (we need to vary in [50,70]) 
            ...
            [Fs_pos,Fs_neg] = getGreedyVals_benchweight(P, Sc, COVIJ);%get sorted positive(Fs_pos) and negative(Fs_neg) indexes of stocks relative to the greedyvals
        ...
        [xtemp, fvaltemp, exitflag] = solve_qp(P, S, COVIJ, Cmat, samplePeriodSize, teta);
        if(isFeasible(itrGRASP, xtemp, fvaltemp, exitflag, P, S))
            ...
            if(rand() <= minLocalSearch)
                ...
                [improved, St, xt, fvalt] = MiLS_TE(itrGRASP, round(0.1*K),...
                    P, S, Sc, xtemp, fvaltemp, COVIJ, Cmat, samplePeriodSize, teta);
