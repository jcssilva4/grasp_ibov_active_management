function [S, Sc] = select_newstocks(P, lambda, COVIJ, sizeRCL, K, S0, Sc0)
    ...
    Scdiv = Sc;
    RCL = []; %Restricted Candidate List (stocks are randomly selected from this list)  
    %PRIORIZATION: you need at least two assets in each mcapQ
    numStockperQ = getNumStockperSec(P, S);
    for q = 1:length(numStockperQ)
        if(numStockperQ(q) < 2)
            %fprintf('\npriority: buy %d assets from %s',secOrder(auxSec,2),P.sectorSet{1,sec});
            [Fs_pos,Fs_neg] = getBuyGreedyVals(P, lambda, P.idxQ{1,q}, COVIJ,1);%get sorted positive(Fs_pos) and negative(Fs_neg) indexes of stocks relative to the greedyvals
