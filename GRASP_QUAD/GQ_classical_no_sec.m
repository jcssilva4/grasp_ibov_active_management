...
function [d,w, fval, Sfinal, COVIJ, covij] = GQ_classical_no_sec(P, lambda, iterMAX)
%{
    Some problems:
...
%}
    ...
    while (itrGRASP <= itrMAX_GRASP)
        ...
        [xtemp, fvaltemp, exitflag] = solve_classical_no_sec(P, lambda, S, COVIJ);
        if(isFeasible(itrGRASP, xtemp, fvaltemp, exitflag, P, S))
            %Minor Local Search
            if(rand() <= minLocalSearch)
                %fprintf('applying MiLS...\n')
                [improved, St, xt, fvalt] = MiLS_no_sec(itrGRASP, round(0.1*K),...
