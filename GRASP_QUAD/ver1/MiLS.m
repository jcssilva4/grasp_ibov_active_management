function [improved, SMiLS, xMiLS, fvalMiLS] = MiLS(itrGRASP, UB_minLocalSearch,...
    ...
    while (i < UB_minLocalSearch)
        ...
        [sec_ok, mcapq_ok, ~, ~] = checkSecMcapQ(P, S, 0, 0);%check
        if(sec_ok + mcapq_ok > 1) %number of secs and mcapq ok!
            [xt, fvalt, exitflag] = solve_qcqp(P, lambda, S, COVIJ);
