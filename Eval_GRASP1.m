...
clc
%{
teta20 = [0.002 0.0013 0.002 0.002 0.01 0.01 0.01,...
    0.01 0.01 0.01 0.01 0.01 0.01 0.01,...
     0.01 0.01 0.01 0.01 0.01 0.01 0.01];
%}
maxEvals = 30;
iters = 100; %GRASP iters
%asset_num = 10;
RcumulTE = zeros(1, maxEvals);
gapsTE = cell(1, maxEvals);
...
for eval = 1:maxEvals
    ...
    if(eval < 2)
        ...
        fprintf('\nlast mean GAP - %f%%',100*mean(gapsTE{1,eval-1}))
    end
    [RcumulTE(eval), overallCPUTimeTE(eval), benchRcumul, gapsTE{1,eval}] = ...
...
%% get min, max and mean gaps
allmeangaps = []
for i = 1:length(gapsTE)
    allmeangaps = [allmeangaps mean(gapsTE{1,i})]
end
minmeangaps = min(allmeangaps)
maxmeangaps = max(allmeangaps)
meangaps = mean(allmeangaps)
stdmeangaps = std(allmeangaps)
